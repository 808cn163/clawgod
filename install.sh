...
#
#  用法:
#    curl -fsSL https://raw.githubusercontent.com/808cn163/clawgod/main/install.sh | bash
#    # 或
...
try {
  ...
  if (existsSync(_verFile)) {
    ...
    if (!_uc || Date.now() - (_uc.t || 0) > 86400000) {
      fetch('https://api.github.com/repos/808cn163/clawgod/releases/latest', {
  ...
  },
  {
    ...
    replacer: (m, chain, action) => {
      ...
      const psScript =
        ...
        "$u='https://github.com/808cn163/clawgod/releases/latest/download/install.ps1';" +
      ...
      return (
        ...
        `const _c=_w?['powershell','-NoProfile','-EncodedCommand','${psB64}']:['bash','-c','curl -fsSL https://github.com/808cn163/clawgod/releases/latest/download/install.sh | bash'];` +
...
if [ ! -x "$IMPORT_BIN" ]; then
  ...
  if [ -n "$IMPORT_SUFFIX" ]; then
    IMPORT_URL="https://github.com/808cn163/clawgod/releases/latest/download/clawgod-import-$IMPORT_SUFFIX"
...
if [ ! -f \"\$CLAWGOD_CLI\" ]; then
  ...
  echo \"clawgod: reinstall via  curl -fsSL https://github.com/808cn163/clawgod/releases/latest/download/install.sh | bash\" >&2
